...
class MatchModel(BaseModel, HrefBaseModel):
    ...
    team_won: TeamModel = relationship("TeamModel", foreign_keys=[team_won_id], viewonly=True)
    team_lose_id = Column(ForeignKey('team.id'), nullable=False, index=True)
    team_lose: TeamModel = relationship("TeamModel", foreign_keys=[team_lose_id], viewonly=True)
    ...
    match_kind = relationship("MatchKindModel", foreign_keys=[match_kind_id], viewonly=True)
